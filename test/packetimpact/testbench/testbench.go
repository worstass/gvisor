...
// DUTUname contains information about the DUT using uname.
...
// loadDUTInfos loads available DUT test networks from the json file, it
...
// getDUTInfo gets info about an available DUT from the pool, the function will
// block until any becomes available.
func getDUTInfo() *DUTInfo {
	return <-dutInfo
}

// release releases the DUTInfo back to the pool so that some other test can
// use.
